...
int main(void)
{
  ...
  tc.base.addMonitor((ControllerSystem*)&tc, (MonitorSystem*)&tp);
  ...
  // Register Inverter for CAN messages (VCU receives ESC status messages)
  if (registerCANReceiver(&can1_comms, "VCU", &inverter, inverter_can_handler, EXTERNAL) == 0) {
  ...
  Updateable* updateables[8];  // Increased size to include CAN systems
